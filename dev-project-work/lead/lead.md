...
| **Find Deepening Opportunities** | Surface architectural friction in the codebase as **deepening candidates** — shallow modules, leaky seams, untestable interfaces. Walks the user through a grilling loop on a chosen candidate using the architecture vocabulary in `lead/improve-architecture/language.md` (Module / Interface / Depth / Seam / Adapter / Leverage / Locality). Side effects flow through existing actions: vocabulary additions through Sharpen Vocabulary discipline, ADR offers through Record Decision. **Full playbook in `lead/improve-architecture/action.md`** — read it when invoking | Design mode; Direct mode; Sprint Planning phase before scoping a refactor chunk | User says "improve architecture", "find deepening opportunities", "what should we refactor", "audit the architecture". User complains about test friction or modules that are hard to navigate. After a code review or vocabulary audit surfaces shallow-module patterns |
