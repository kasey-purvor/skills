...
The repo's issue-tracker binding (Linear team + issue prefix) lives in `AGENTS.md` (`## Agent skills` → `### Issue tracker`) — run `/setup-matt-pocock-skills` if that block is missing.
...
1. Issue references in the commit messages (`CLO-123`, `Closes CLO-45`, etc.) — fetch via the `linear` skill: `get_issue` **and** `list_comments` (a ticket is two calls — description plus discussion).
